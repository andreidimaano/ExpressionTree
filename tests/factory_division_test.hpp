...
//1 / 0
TEST(ClassDivisionTest, DivEvaluateZero) {
...
// //1 / 2
TEST(ClassDivisionTest, DivEvaluateNonZero) {
  Factory* test = new Factory();
  char** input = new char*[4];
  ...
  Base* expression = new Div(decimal_one, decimal_two);

  EXPECT_EQ(test->parse(input, 4)->evaluate(), expression->evaluate()); 
}

TEST(ClassDivisionTest, DivEvaluateNonZeroString) {
  Factory* test = new Factory();
  char** input = new char*[4];
  ...
  EXPECT_EQ(test->parse(input, 4)->stringify(), expression->stringify()); 
}

//1 + 1 + 1 + 1
TEST(ClassDivisionTest, DivMultipleEvaluateNonZero) {
  Factory* test = new Factory();
  ...
  Base* decimal_one = new Op(20);
  Base* decimal_two = new Op(2);
  Base* expression_one = new Div(decimal_one, decimal_two);
  //1 + 1
  Base* decimal_three = new Op(2);
  Base* expression_two = new Div(expression_one, decimal_three);
  //1 + 1 + 1

  Base* decimal_four = new Op(2);
  Base* expression_three = new Div(expression_two, decimal_four);
  //1 + 1 + 1 + 1
  EXPECT_EQ(test->parse(input, 8)->evaluate(), expression_three->evaluate()); 
}

// 1 +
TEST(ClassDivisionTest, DivInvalidEndWithOperator) {
  Factory* test = new Factory();
...
TEST(ClassDivisionTest, DivInvalidStartWithOperator) {
  Factory* test = new Factory();
  char** input = new char*[3];
...
// // 1a +
TEST(ClassDivisionTest, DivMixedOperand) {
  Factory* test = new Factory();
  char** input = new char*[4];
...
// a
TEST(ClassDivisionTest, DivInvalidOperand) {
  Factory* test = new Factory();
  char** input = new char*[4];
